...
// Shamelessly copied from the god himself, @andruzzzhka
namespace BeatSaberKeyboardMapperPlugin.UI
{
    class BeatSaberUI : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            try
            {
                _buttonInstance = Resources.FindObjectsOfTypeAll<Button>().First((Button x) => x.name == "QuitButton");
                _backButtonInstance = Resources.FindObjectsOfTypeAll<Button>().First((Button x) => x.name == "BackArrowButton");
                _loadingIndicatorInstance = (from x in Resources.FindObjectsOfTypeAll<GameObject>()
                                             where x.name == "LoadingIndicator"
                                             select x).First();
            }
            catch (Exception e)
            {
                Logger.log.Error($"EXCEPTION ON AWAKE(TRY FIND BUTTONS): {e}");
            }
